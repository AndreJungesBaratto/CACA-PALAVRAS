...
//                        %c%c%c%c%c%c 6    %c%c%c%c%c%c%c%c 7   %c%c 2
...
void entradados(dados v[N]) {
    ...
    while(strlen(v[cont].palavra)<=1||igual==0||strpbrk(v[cont].palavra, numero)!=NULL){
        ...
        if(strpbrk(v[cont].palavra, numero) != NULL){
            ...
            system("color 7");
            system("cls");
            printf(BYEL"Por favor, insira uma palavra com mais de uma letra: "RST);
        }
        if(igual == 0){
            system("color 6");
            Sleep(500);
...
void teladefim(){
    ...
    getchar();
    system("cls");
    printf(BYEL"Aperte enter para voltar ao menu."RST"\n");
    getchar();
}
